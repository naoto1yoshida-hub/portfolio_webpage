...
- **図面検索システム** — PostgreSQL + pgvector と Vercel Blob が要り、ローカルでは起動できません。
  構成図のみで掲載しています。撮るならローカルに pgvector 入り Postgres を用意する必要があります。
- **品質管理チャットボット / 音声入力アプリ** — 未着手。前者は Excel 取込済みDBが要り、
  後者は Windows デスクトップアプリなので撮影手順が別になります。
